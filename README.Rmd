...
`arc_open()` takes a URL to create a reference to a remote resource. The function can return any of four classes (corresponding to different ArcGIS service types):
...
You can then query the feature layer object and return an `sf` object using `arc_select()`.
...
The `list_fields()` function can be helpful to check available attributes and build a `where` query:
