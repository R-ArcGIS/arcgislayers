...
- `arc_open()` creates a reference to a dataset will return 1 of 4 types of objects:
  - `FeatureLayer`
  - `Table`
  - `FeatureServer`
  - `ImageServer`
...
Query the feature layer object and return an `sf` object using `arc_select()`. If no arguments are provided to `arc_select()` the entire feature layer is returned in memory as an `sf` object. 
...
Note you can also provide an `bbox`, `sfc`, or `sfg` object to the `filter_geom` argument to perform a spatial filter. If the `sfc` object contains more than one geometry, the object is combined with `sf::st_union()`. See documentation for more (`?arc_select`).
